...
with synchronizers, like link:https://www.dropbox.com[Dropbox], these settings can be used across
multiple machines.
...
_The install assumes this GitHub project will be kept locally since the Sublime Text `Users` folder
will be symbolically linked to this project. Should this project be relocated, re-run the install to
fix the symbolic link._ Alternatively, the install can be skipped and content can be pilfered from
the files located in the `preferences/User` folder of this project instead.
...
* Benchmark IPS - `bmi`
* Benchmark Mem - `bmm`
* Benchmark Report (multi-line) - `reportm`
